...
ostream& operator<<(ostringstream& s, const pair<string, string>& x)
{
    s << x.left << " " << x.right;
    return s;
}
...
class HashSetTable{
public:
    int default_table_size = 1e3;
    int default_hash_key = 97;
...
public:
    ...
    vector<HashSetTable<pair<T1, T2> > table;
...
public:
    ...
    bool insert(T1 key, T2 value){
        ...
        table[hash].insert(make_pair(key, value));
    }
    ...
    bool remove(T1 key, T2 value){
        table[hash].remove(make_pair(key, value))
    }
...
int32_t main(){
    ...
    HashMultiMapTable<string, string> t;
